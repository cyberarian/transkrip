...
export function TasksPage({ ownerId }: { ownerId: number }) {
  ...
  const exportWord = async (row: TranscriptionSummary) => {
    setBusyId(row.id)
    try {
      await autosave.flush()
      const record = await getTranscription(row.id)
      const { transcriptDocx, saveDocx } = await import('../docx-export')
      const blocksText = record.formattedTranscript.map(block => `${block.speakerLabel}:\n${block.text}`).join('\n\n')
      const text = record.correctedText || record.rawText
      const segments = blocksText.trim() === text.trim() ? record.formattedTranscript : [{ text }]
      saveDocx(await transcriptDocx({ title: 'Transkrip', source: record.audioFile, language: record.language, segments }), `transkrip-${record.id}.docx`)
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Dokumen Word gagal dibuat.') }
    finally { setBusyId(null) }
  }
